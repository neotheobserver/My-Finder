...
RAYGUIAPI int GuiListViewCustom(Rectangle bounds, int count, int *scrollIndex, int *active, int *focus);
...
int GuiListViewCustom(Rectangle bounds, int count, int *scrollIndex, int *active, int *focus)
{
    ...
    int counter = 0;
    while (counter != startIndex && list != NULL)
    {
        list = list->next;
        counter++;
    } 
    // Draw visible items
    for (int i = 0; ((i < visibleItems) && (list != NULL)); i++)
    {
        if (state == STATE_DISABLED)
        {
            ...
            GuiDrawText(list->result->string, GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_DISABLED)));
        }
        else
        {
            if (((startIndex + i) == itemSelected) && (active != NULL))
            {
                ...
                GuiDrawText(list->result->string, GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_PRESSED)));
            }
            else if (((startIndex + i) == itemFocused)) // && (focus != NULL))  // NOTE: We want items focused, despite not returned!
            {
                ...
                GuiDrawText(list->result->string, GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_FOCUSED)));
            }
            else
            {
                // Draw item normal
                GuiDrawText(list->result->string, GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_NORMAL)));
            }
        }
        list = list->next;
        // Update item rectangle y position for next item
